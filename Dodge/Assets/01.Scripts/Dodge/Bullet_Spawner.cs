...
public class Bullet_Spawner : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        target = FindObjectOfType<Player_controller>().transform;   // 씬에 존재하는 모든 오브젝트들을 검사하여 지정한 타입과 일치한 오브젝트를 가져오는 거기 때문데 여러번 실행시킬 경우 프로그램이 심각하게 느려질수있습니다.
        // FindObjectOfType 메서드는 지정한 타입의 오브젝트가 여러명이고 그 오브젝트들을 가져올 때 사용된다.
    }
