...
public class GameManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        best_score = PlayerPrefs.GetInt("Best_score", 0); // 추가: 이전 최고 점수를 불러옴
    }
    ...
    public void EndGame()
    {
        ...
        if (itemAcount > best_score) // 추가: 현재 점수가 최고 점수보다 높으면 갱신
        {
            best_score = itemAcount;
            PlayerPrefs.SetInt("Best_score", best_score); // 추가: 최고 점수를 저장
        }

        record_text.text = "Best Score: " + best_score; // 수정: 최고 점수를 표시
    }
}
